 ...
 *
 *	tms7000 - Nice and simple as we use the compiler calling convention
...
{
  FILE *fp;
  snprintf(namebuf, 128, "fuzixtms7000/syscall_%s.s",syscall_name[n]);
  fp = fopen(namebuf, "w");
  if (fp == NULL) {
    perror(namebuf);
    exit(1);
...
static void write_makefile(void)
{
  ...
  fp = fopen("fuzixtms7000/Makefile", "w");
  if (fp == NULL) {
    perror("Makefile");
    exit(1);
  }
  fprintf(fp, "# Autogenerated by tools/syscalltms7000\n");
  /* For now name it differently until we do the flip properly */
  fprintf(fp, "ASYS=syscall_tms7000.s\n");
