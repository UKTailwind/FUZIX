...
{
  FILE *fp;
  snprintf(namebuf, 128, "fuzixz8/syscall_%s.s",syscall_name[n]);
  fp = fopen(namebuf, "w");
  if (fp == NULL) {
    perror(namebuf);
    exit(1);
...
static void write_makefile(void)
{
  ...
  fp = fopen("fuzixz8/Makefile", "w");
  if (fp == NULL) {
    perror("Makefile");
    exit(1);
  }
  fprintf(fp, "# Autogenerated by tools/syscallz8\n");
  /* For now name it differently until we do the flip properly */
  fprintf(fp, "ASYS=syscall_z8.s\n");
