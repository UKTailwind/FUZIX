  ' PicoFrog by Martin Herhaus (@harm), from the PicoMite community,
  ' ported to the Pico Computer 3 under Fuzix.  Every line this port
  ' changed is marked PORT: and the original is beside it, so the diff
  ' from picofrog6_b9.bas stays readable.
  '
  ' What the port did, and why:
  '   - KEYBOARD ONLY.  The NES socket, the Game*Mite keypad and the
  '     Wii nunchuck are not fitted here, and the MM.INFO(PinNo ...),
  ...
  '     there to avoid jumping out of a Sub; we have no RUN.
  '   - two defects the interpreter tolerated: one ')' too many, and an
  '     @harm annotation that was never made a comment.
  '
  '@harm
  'status
  '   b1   game works, keyboard control,framebuffer merge+music sync'd
  '   b2   gamemite keys implemented
  '   b3   music note adsr changed to support 2040 @ 252MHz
  '   b4   @fly and generic LCD support
  '   b5   fixed fly bugs, know bug: high score lost when RUN
  '   b6   removed lady frog, VGA and LCD and Game*Mite
  '   b7   wii, nunchuck and nes controller work
  '   b8   fix wii, add sprites,crocodile + turtle dive
  '   b9   framework levels
  
  '@harm todo list
  ' - repair high score
  ' - implement croc in home position
  ' - implement lady frog
  ' - check reward system against real game
  ' - check sounds against real game
  ' - make levels...
  ...
  'controllers
  const no_nes%=1 'PORT: no NES socket on this machine
  'PORT: Const nes_data% = Mm.Info(PinNo GP1) 'define needed, NES is not auto detect
  'PORT: Const nes_latch% = Mm.Info(PinNo GP2)
  'PORT: Const nes_clock% = Mm.Info(PinNo GP3)
  'PORT: Const nes_pulse! = 0.012 ' 12uS
  const gm%=0 'PORT: not a GameMite
  dim is_wii%=0,ad_wii%=&h52,iic_wii%,is_nes%=0
  ...
  'PORT: keyboard is the only controller here.
  'PORT: init_kb 'initialize GM keys
  'PORT: else
  'PORT: 'check NES
  'PORT: if nes_possible%() then nes_init : is_nes%=1 'init the port, confirm in startscreen
  'PORT: 'check WII
  'PORT: if instr(mm.info(system I2C),"I2C") then find_wii
  'PORT: end if
  ...
  bing
  if lcd% then if lcd% then framebuffer merge 0,b
  framebuffer write lc$
  read_Sprites
  Box 0,0,224,24,,0,0
...
Sub GameOver
  text 112,112,"GAME OVER",CM,,2,rgb(MYRTLE),rgb(white)
  if lcd% then framebuffer merge 0,b
  
  Pause 10000
  END 'PORT: MMBasic RUN restarts the program; we have no RUN
End Sub
...
Sub ShowLives
  ...
  For f%=1 To Lives%
    Sprite write FPOSP%+1,222+f%*16,144
  Next f%
End Sub
...
Sub startscreen
  ...
  Loop until inkey$=" " 'PORT: keyboard only
  'PORT: if nes_read$()="A " then is_nes%=2 'confirm the NES controller is used here
...
FMout:
  ...
  Select Case inkey$
      ...
      Case chr$(130) : PLD$="L"
      Case chr$(131) : PLD$="R"
      Case Else : PLD$ = "" 'PORT: keyboard only
  End Select
...
Sub read_Sprites
  ...
  For nr%=1 To 6
    For p%=1 To 8
      ...
      For n%=1 To Len(Byt$)
        ...
        Pixel n%-1,136-p%,COL%(Val("&H"+m$))  'PORT: had one ) too many
      Next n%
    Next p%
  ...
  restore home_slot '@harm  'PORT: @harm was not a comment
  ...
  'test 64 sprites
  box 0,120,16,16,,rgb(blue),rgb(blue)
...
sub loadlevel n%
  ...
  'PORT: init_kb, rd_kb$, find_wii, check_id%, read_wii$,
  'PORT: nes_possible%, nes_init and nes_read$ removed - NES,
  'PORT: GameMite keypad and Wii nunchuck are not fitted here,
  'PORT: and the MM.INFO calls that hunt for them have no
  'PORT: answer on this machine.  The keyboard does it all.
