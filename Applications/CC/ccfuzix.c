 ...
 *	Split I/D
 *
 *	Merge together with ifdefs the different forms
 *	Move to allowing 8080 builds on 8085 or Z80 etc
 */
...
#define CMD_CC2		LIBPATH"cc2"
#define CMD_COPT	LIBPATH"copt"
#define CMD_CPP		BINPATH"cpp"
#define CMD_LD		BINPATH"ld"
...
#else
	/* Decus CPP is a bit quirky */
	build_arglist(CMD_CPP);
	add_argument(path);
	add_argument("-o");
	tmp = xstrdup(path, 0);
	add_argument(pathmod(tmp, ".S", ".s", 1));
#endif	
	run_command();
	pathmod(path, ".S", ".s", 5);
}
...
void preprocess_c(char *path)
{
	...
	add_argument(path);
	tmp = xstrdup(path, 0);
/*	if (last_phase != 1) FIXME */
	add_argument("-o");
	add_argument(pathmod(tmp, ".c", ".%", 0));
#endif
	run_command();
}
