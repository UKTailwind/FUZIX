' BMP180 pressure and temperature on the FIXED I2C bus - the QWIIC
' socket, GP20/GP21.
'
' The same program as hwtest/bmp180.bas, which runs it on I2C2 over the
' I/O header.  The ONLY differences are the bus: no SETPIN, no I2C OPEN
' and no I2C CLOSE, because GP20/GP21 are the board's and the
' controller is already running for the DS3231.  Everything below - the
' calibration read, the Bosch fixed-point arithmetic, STR2BIN over the
' received string - is the original, unchanged.
'
' It opens with I2C CHECK, which is what that statement is for: no
' transfer here raises, so a sensor that is not plugged in would
' otherwise be read as a wall of zeroes.
'
...
main:
  ...
  ' (nothing to close: the fixed bus is always open)
end
'
