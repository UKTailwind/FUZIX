...
/* FS32 fsck, on-card edition.  The format is defined by
 * Kernel/platform/platform-rpipico/FS32-FORMAT.md; this keeps its own
 * struct copies ON PURPOSE - it is one of the independent
 * implementations cross-checked against the kernel and the host tools.
 *
 * Ported from the classic fsck-fuzix with the same fixes as the host
 * fsck: repairs that wrote to the wrong block in the pass-1 indirect
 * zap and setblkno's double-indirect path, and out-of-range pointers
 * are never followed or mapped (the bounds-guarded bitset here already
 * prevented the bitmap overrun the host version had).
 *
 * Memory: the allocation bitmap is fsize/8 bytes.  A 65536-block card
 * partition needs 8K, fine; a filesystem too big for the process just
 * fails the calloc and reports it - check those from the host instead.
 */
...
static const char no_memory[] = { "Not enough memory (use the host fsck for big filesystems).\n" };
...
static char *daread(blkno_t blk);
static void dwrite(blkno_t blk, char *addr);
...
{
    return b >= superblock.s_isize && b < superblock.s_fsize;
}
...
int perform_fsck(char *name)
{
    ...
    if (superblock.s_mounted == SMOUNTED_CLASSIC) {
        puts("Classic (pre-FS32) filesystem - reformat needed.");
        return (error |= 32);
    }
    ...
    if (superblock.s_mounted != SMOUNTED) {
        ...
        superblock.s_version = FS32_VERSION;
        dwrite((blkno_t) 1, (char *) &superblock);
    }

    if (superblock.s_version != FS32_VERSION || superblock.s_shift != 0) {
        printf("FS32 version %u shift %u not supported by this fsck.\n",
                superblock.s_version, superblock.s_shift);
        return (error |= 32);
    }

    printf("Device %u has fsize = %lu and isize = %lu. Continue? ",
            dev, (unsigned long) superblock.s_fsize,
            (unsigned long) superblock.s_isize);
    if (!yes_noerror())
        return (error |= 32);

    linkmap = (int16_t *) calloc(IPERBLK * superblock.s_isize, sizeof(int16_t));
    ...
    max_inode = IPERBLK * (superblock.s_isize - 2);
 ...
 * given root, zapping out-of-range entries.  depth 1 for a double
 * root, 2 for a triple root; data blocks are covered by the getblkno
 * walk.  Uses a private copy per level because daread keeps only one
 * block. */
static void mark_middles(uint16_t n, blkno_t root, int depth)
{
    ...
    for (b = 0; b < IND_PER_BLOCK; ++b) {
        ...
        progress();
        if (!blk_in_range(e)) {
            printf("Inode %u indirect chain entry %u out of range, val = %lu. Zap? ",
                    n, b, (unsigned long) e);
            if (yes()) {
                char *buf = daread(root);
                ((blkno_t *) buf)[b] = 0;
                dwrite(root, buf);
                local[b] = 0;
            }
            /* Whether zapped or kept, never follow a wild pointer */
            continue;
        }
        bitset(e);
...
static void pass1(void)
{
    ...
    uint16_t icount;
    uint32_t bmax;
    /* Lowest size at which each indirect root may be nonzero */
    static const uint32_t root_threshold[3] = {
        DIRECT_BLOCKS * 512UL,
        (DIRECT_BLOCKS + IND_PER_BLOCK) * 512UL,
        (DIRECT_BLOCKS + IND_PER_BLOCK +
         (uint32_t)IND_PER_BLOCK * IND_PER_BLOCK) * 512UL
    };
    ...
    for (n = ROOTINODE; n < max_inode; ++n) {
        ...
        linkmap[n] = 0;
        ++icount;
        /* Check size: off_t is int32 */

        if (ino.i_size > 0x7FFFFFFFUL) {
            printf("Inode %u size exceeds off_t with value of %lu. Fix? ",
                    n, (unsigned long) ino.i_size);
            if (yes()) {
                ino.i_size = 0;
                iwrite(n, &ino);
            }
        ...
        if (mode == F_REG || mode == F_DIR) {
            ...
            for (b = DIRECT_BLOCKS; b < DIRECT_BLOCKS + 3; ++b) {
                if (ino.i_addr[b] != 0 &&
                    !blk_in_range(ino.i_addr[b])) {
                    printf("Inode %u indirect root %u out of range, val = %lu. Zap? ",
                            n, b, (unsigned long) ino.i_addr[b]);
                    if (yes()) {
                        ino.i_addr[b] = 0;
                        iwrite(n, &ino);
                    }
                ...
                if (ino.i_addr[b] != 0 &&
                    ino.i_size < root_threshold[b - DIRECT_BLOCKS]) {
                    printf("Inode %u indirect root %u past end of file, val = %lu. Zap? ",
                            n, b, (unsigned long) ino.i_addr[b]);
                    if (yes()) {
                        ino.i_addr[b] = 0;
                        iwrite(n, &ino);
                ...
                if (ino.i_addr[b] != 0 && blk_in_range(ino.i_addr[b]))
                    bitset(ino.i_addr[b]);
            }

            /* Middle blocks of the double and triple trees; the
               single root's children are data blocks and the walk
               below covers them */
            mark_middles(n, ino.i_addr[DIRECT_BLOCKS + 1], 1);
            mark_middles(n, ino.i_addr[DIRECT_BLOCKS + 2], 2);
            ...
            for (bno = 0; bno <= bmax ; ++bno) {
                ...
                if (db != 0 && !blk_in_range(db)) {
                    printf("Inode %u block %lu out of range, val = %lu. Zap? ",
                            n, (unsigned long) bno, (unsigned long) db);
                    if (yes()) {
                        setblkno(&ino, bno, 0);
                        iwrite(n, &ino);
                    }
...
static void pass2(void)
{
    blkno_t j;
    blkno_t oldtfree;
    blkno_t imax = superblock.s_isize;
    ...
    if (oldtfree != superblock.s_tfree)
        printf("s_tfree was changed to %lu from %lu.\n",
                (unsigned long) superblock.s_tfree,
                (unsigned long) oldtfree);

}

/* Pass 3 finds and fixes multiply allocated blocks.  Covers the
   indirect roots and the data blocks, as classic did. */
static void pass3(void)
{
    uint16_t n;
    struct dinode ino;
    ...
    for (db = 0; db < superblock.s_isize; db++)
        bitset(db);
...
static blkno_t getblkno(struct dinode *ino, blkno_t num)
{
    ...
    if (num < (blkno_t) IND_PER_BLOCK * IND_PER_BLOCK) {  /* Double */
        indb = ino->i_addr[DIRECT_BLOCKS + 1];
        if (indb == 0)
            return 0;
        buf = (blkno_t *) daread(indb);
        dindb = buf[num >> 7];
        ...
        return buf[num & (IND_PER_BLOCK - 1)];
    }
    /* Triple indirect */
    num -= (blkno_t) IND_PER_BLOCK * IND_PER_BLOCK;
    indb = ino->i_addr[DIRECT_BLOCKS + 2];
    if (indb == 0)
        return 0;
    buf = (blkno_t *) daread(indb);
    dindb = buf[num >> 14];
    if (dindb == 0)
        return 0;
    buf = (blkno_t *) daread(dindb);
    dindb = buf[(num >> 7) & (IND_PER_BLOCK - 1)];
    if (dindb == 0)
        return 0;
    buf = (blkno_t *) daread(dindb);
    return buf[num & (IND_PER_BLOCK - 1)];
 ...
 *  disk block number in an EXISTING chain: a missing indirect block is
 *  a panic, never a creation.
 */
static void setblkno(struct dinode *ino, blkno_t num, blkno_t dnum)
{
    blkno_t indb;
    ...
    if (indb == 0)
        panic(missing_ind_blk);
    buf = (blkno_t *) daread(indb);
    indb = buf[(num >> 7) & (IND_PER_BLOCK - 1)];
    if (indb == 0)
        panic(missing_ind_blk);
    buf = (blkno_t *) daread(indb);
    buf[num & (IND_PER_BLOCK - 1)] = dnum;
...
static char *daread(blkno_t blk)
{
    ...
    if (lseek(dev_fd, (off_t) blk * 512, 0) == -1) {
        perror("lseek");
        exit(1);
    }
...
static void iread(uint16_t ino, struct dinode *buf)
{
    ...
    memcpy(buf, &addr[ino & 1], sizeof(struct dinode));
}
...
static void iwrite(uint16_t ino, struct dinode *buf)
{
    ...
    addr = (struct dinode *) daread((ino >> 1) + 2);
    memcpy(&addr[ino & 1], buf, sizeof(struct dinode));
    dwrite((ino >> 1) + 2, (char *) addr);
}
