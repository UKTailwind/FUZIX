' PicoMan for the Pico Computer 3.
'
' Geoff Graham's PacMan for the PicoGamer and Game*Mite, with the menu
' removed and the eight switches on GP8-GP15 replaced by the keyboard -
' this machine cannot claim those pins at all.
'
'   arrow keys   move        space   A        b   B
'   s            start       q or e  select / exit
'
' (c) 2023 Geoff Graham.
Mode 2
...
' (the MMBasic-for-Windows arm is gone: both arms declared the same
'  global CONSTs, and a compiler has to pick one)
  ...
  Dim   GhostSlow = 29   ' Speed of the Ghosts when the power ball is active
  ' init the button inputs
  ' The switch array moved to GP34-GP41: GP8-GP15 are not
  ' claimable on this machine.  Same bit order, same active-low
  ' wiring, internal pull-ups.  A board with no switches reads
  ' these as all-high, which is 'nothing pressed'.
  For i = 34 To 41
    SetPin i, DIn, PullUp
  Next i
  FrameBuffer Create
...
' get a key input
Function GetKey()
  ' The Game*Mite read all eight switches at once as
  '     (Port(GP8, 8) Xor &h7FFF) And &hFF
  ' with GP8 the lowest bit.  GP8-GP15 cannot be claimed on this
  ' machine, so the switch array is on GP34-GP41 in the same bit order
  ' and the read is otherwise identical.
  '
  ' BOTH sources, OR'd: switches AND keyboard.  No detection, because
  ' there is nothing reliable to detect - an unwired pin with its
  ' pull-up reads exactly like an unpressed switch.  A machine with no
  ' switch array simply contributes nothing from the port, and one with
  ' switches can still be driven from the keyboard.
  '
  ' Faithful downstream: the game compares this with `=`, so two
  ' buttons at once match neither.  That is the original's behaviour on
  ' a Game*Mite and it is left alone.
  Local integer i, n, k
  GetKey = (Port(GP34, 8) Xor &hFF) And &hFF
  n = KeyDown(0)
  For i = 1 To n
    k = KeyDown(i)
    If k = 129 Then GetKey = GetKey Or bDOWN
    If k = 130 Then GetKey = GetKey Or bLEFT
    If k = 128 Then GetKey = GetKey Or bUP
    If k = 131 Then GetKey = GetKey Or bRIGHT
    If k = 101 Or k = 113 Then GetKey = GetKey Or bSELECT
    If k = 115 Then GetKey = GetKey Or bSTART
    If k = 98 Then GetKey = GetKey Or bB
    If k = 32 Then GetKey = GetKey Or bA
  Next i
End Function

' SELECT used to load the menu; there is none, so it stops.
Sub end_program
  FrameBuffer Write N
  FrameBuffer Close
...
Function QuitCheck()
  ...
  Do
    ...
    Do
      ...
      ElseIf key = bSELECT Or key = bSTART Then
        MakeSound ack()
        Do : key = GetKey() : Loop While key <> 0
        If state = 0 Then
