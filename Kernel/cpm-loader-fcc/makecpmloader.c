...
int main(int argc, char **argv)
{
    ...
    trailer.load_length = kernel_length-load_address;
    trailer.load_address = load_address;
#else
    trailer.load_length = BYTESWAP16(kernel_length-load_address);
    ...
    /* Skip 4 bytes in the end of the loader, and skip load_address number of bytes
     * in the beginning of the kernel due to differences in the binary format 
     * with fcc linker */
    if(write(fd, &loader_data[LOADER_TRIM], loader_length - LOADER_TRIM - 4) != (loader_length - LOADER_TRIM - 4) ||
       write(fd, &trailer, sizeof(trailer)) != sizeof(trailer) ||
       write(fd, &kernel_data[load_address], kernel_length-load_address) != kernel_length-load_address){
