...
**TinyUSB must be 0.21.** It was 0.20 — the version the PC3's MMBasic
firmware uses — until 2026-09-01, when 0.20's RP2040/RP2350 host
controller driver was found to be the reason keyboards drop off the bus
at boot: it retried a NAKing device every 16 µs with no way to yield
...
version; `usbcheck.sh` asserts 0.21. SDK 2.3.0 bundles 0.18; after the
SDK is fetched (or in a local SDK), upgrade its copy in place — the
bundled tree is a git checkout, so it is one command:

    git -C $SDK/lib/tinyusb fetch --depth 1 origin tag 0.21.0
    git -C $SDK/lib/tinyusb checkout -f 0.21.0
...
drops the SDK's copy from `tinyusb_host_base` and compiles
`build/hcd_rp2040_pc3.c`, generated by `patch(1)` from the SDK's file;
nothing in the SDK tree is edited, and if its TinyUSB ever changes under
the hunk, `patch` refuses and the build stops there. `patch` is
therefore a build dependency alongside `make`, `cmake` and `git`. The
